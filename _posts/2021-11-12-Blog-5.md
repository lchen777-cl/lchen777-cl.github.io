...
Everytime we call `train_dataset.take(1)` we can retrieve one batch, for our case are labeled 32 imagies from the dataset. Now to try out working with the dataset, let's write a function generating three random picutres of dogs and three random pictures of cats.
